/* YX DOR
 * Works on any regular topology from 1 to N dimensions
 * (not limited to 2D meshes)
 */

#include "ocin_router_ru_yxdor.h"


void ocin_router_ru_yxdor::init(ocin_router *parent, const string &_name) {
 ...
 */
void ocin_router_ru_yxdor::route (ocin_flit *flit) {
	...
	// clear the old set of routes
	msg->routes.clear();
	
	// 1. figure out the dimension for next hop
	int last_dim = _coord.size() - 1; 
	while (msg->dest_addr[last_dim] == _coord[last_dim]) {
	...
	if (done) {
		...
		rt_dir = (msg->dest_addr[last_dim] > _coord[last_dim]) ? INC : DEC ; 
	}
	
	// 2. map the dimension to an output port
	// Note: the mapping assumes that local port is the LAST port.
	p_out = done ? /*ej port*/ port_count : get_port(last_dim, rt_dir);
...
//msg->rt_dimension = last_dim;
//msg->rt_dir = rt_dir;
//msg->vc_set = vc_out_set;
//msg->p_out = p_out;
	
	msg->routes[p_out] = vc_out_set;
