/* Non-speculative 2-level XBar allocator for EC/BECs.
...
void ocin_router_xballoc_bec_2level::init(ocin_router *parent, const string &_name) {
 ...
 */
void ocin_router_xballoc_bec_2level::allocate_xbar() {
	int i;
	int lvc_in = 0; 
	...
	for (int lp_in=0; lp_in < lports; lp_in++) {
	  ...
	  // iterate over classes (vc_fifos_j) here
	    ...
	    for (lvc_in = 0; lvc_in < _lvc_fifos[lp_in].size(); lvc_in++) {
			...
			if (!_lvc_fifos[lp_in][lvc_in]->needs_xbar() )  // non-speculative (has_vc=true)
				continue;
			...
			int vc_out_idx = _lvc_fifos[lp_in][lvc_in]->vc_out_index;
                        ...
                        {
                          stringstream tmp;
                          tmp  << "request: msg #" << _lvc_fifos[lp_in][lvc_in]->front_msg()->pid
                          	   << ". lp_in " << lp_in  << " (vin lidx = " << lvc_in << "). " 
	...
	for (pout2pin_itr = map_pout2pin.begin(); pout2pin_itr != map_pout2pin.end(); pout2pin_itr++) {
		...
		int p_out = lvc_in_ptr->p_out;
		int vc_out_idx = lvc_in_ptr->vc_out_index;
		ocin_vc_status *v = &(parent_router->downstream_vc_stats[p_out][vc_out_idx]);
		bool end_of_msg = lvc_in_ptr->front_flit()->is_tail;
		v->dec_credits(end_of_msg);

		// 4. Generate upstream credit
		if (lvc_in_ptr->credit_port != NULL) {
			gen_credit_update(lvc_in_ptr);
		}
		else {
#ifdef DEBUG
                  {
                    stringstream tmp;
                    tmp  <<"No upstream credit from p_in " << lp_in 
		...
		_xbar->enque(lvc_in_ptr);
                ...
                {
                  stringstream tmp;
                  tmp  << "Winner!!! msg #" << lvc_in_ptr->front_msg()->pid
                       << ". lp_in = " << lp_in << ", lvc_idx = " << lvc_in_index
                       << "(src vc_id: " << src_vc_id << "). "  
                       << "lp_out = " << lp_out << " (vc_out idx = " << lvc_in_ptr->vc_out_index <<"). ";
                  tmp << ". Status:" << endl;
 ...
 */
inline void ocin_router_xballoc_bec_2level::gen_credit_update(ocin_router_vc_fifo *vc_in_ptr) {
